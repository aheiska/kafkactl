...
contexts:
  ...
  remote-cluster:
    ...
    tls:
      ...
      cert: my-cert
      certKey: my-key
      # optional: passphrase for encrypted cert key (if omitted and key is encrypted, will prompt or use keyring)
      certKeyPassphrase: my-passphrase
    ...
    sasl:
      enabled: true
      username: admin
      # optional: if omitted, password is looked up in the OS keyring or prompted interactively
      password: admin
...
# optional: keyring integration for credential storage (defaults to enabled)
keyring:
  # set to false to disable OS keyring lookup and storage for passwords/passphrases
  enabled: true
----
...
kafkactl supports multiple strategies for supplying passwords — for SASL authentication, TLS cert key passphrases, and Schema Registry credentials.

=== Interactive Prompting

If a required credential is not present in the config file or environment, kafkactl prompts for it on the terminal:
...
After a password is entered interactively, kafkactl stores it in the OS keyring (GNOME Keyring on Linux, Keychain on macOS, Credential Manager on Windows) on successful command completion. Subsequent invocations retrieve the password from the keyring without prompting.

Keyring storage is enabled by default. To disable it:
...
To clear a stored credential and re-prompt (e.g. after a password rotation), use the `--clear-keyring` flag:
...
This deletes the existing keyring entry before prompting and stores the newly entered value on success.

=== Credentials in Config File

Credentials can be stored directly in the config file. This is convenient for development but not recommended for production environments:
...
contexts:
  my-cluster:
    sasl:
      enabled: true
      username: my-user
      password: my-password   # stored in plaintext
    tls:
      certKey: my-key.pem
      certKeyPassphrase: my-passphrase   # stored in plaintext
----

=== Kubernetes Secrets

When running in Kubernetes mode, use `saslSecret` to avoid passing credentials as plaintext environment variables in the pod.
See <<Running in Kubernetes>> for the full `saslSecret` configuration options.
